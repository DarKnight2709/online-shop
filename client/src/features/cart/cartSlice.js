...
export const cartSlice = createSlice({
    ...
    reducers: {
        ...
        changeSingleQuantity: (state, action) => {
             state.cartItems[action.payload.cartitemid].quantity = action.payload.quantity; 
             let newTotal = 0.00;
             for(const item in state.cartItems) {
                newTotal += state.cartItems[item].price * state.cartItems[item].quantity;
             }
