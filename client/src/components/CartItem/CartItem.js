    ...
    return (
        <div className="row gx-2 pb-4 mt-3 border-bottom">
            ...
            <div className="col-6 col-md-7 cart-item mt-lg-3">
                ...
                <p className="card-text">{`${formatVND(itemObj.price)}`}</p>
                <FontAwesomeIcon
                    title="Xoá sản phẩm"
                    className="cart-remove"
                    onClick={() => removeHandler(itemObj.productid)}
                    icon={faTrashCan}
                />
            </div>
            ...
            <div className="col-3 d-flex cart-count-parent justify-content-end">
                <button
                    onClick={() => decrementhandler(itemObj.productid)}
                    className="btn btn-secondary px-auto py-0 rounded-0 text-center"
                    aria-label="Giảm số lượng"
                >-</button>
                <input
                    data-id={itemObj.productid}
                    className="form-control rounded-0 px-0 text-center"
                    type="number"
                    value={qty}
                    onChange={(e) => handleChange(e.target.value, itemObj.productid)}
                    onBlur={(e) => handleUnfocus(e, itemObj.productid)}
                    aria-label="Số lượng sản phẩm"
                />
                <button
                    onClick={() => incrementHandler(itemObj.productid)}
                    className="btn btn-secondary px-auto py-0 rounded-0 text-center"
